...
const Store = {
  ...
  DB_VERSION: 1,
  db: null,
  LS_RECORDS: 'hl_records',
  LS_CONFIG: 'hl_config',
  LS_USERS: 'hl_users',
  LS_SPACES: 'hl_spaces',
  LS_INVITATIONS: 'hl_invitations',
  ...
  /* ---- 配置管理 ---- */
  getConfig() {
    try { return JSON.parse(localStorage.getItem(this.LS_CONFIG)); }
    catch { return null; }
  },
  saveConfig(config) {
  ...
  /* ---- 用户管理 ---- */
  getUsers() {
    try { return JSON.parse(localStorage.getItem(this.LS_USERS)) || []; }
    catch { return []; }
  },
  saveUsers(users) {
  ...
  /* ---- 空间管理 ---- */
  getSpaces() {
    try { return JSON.parse(localStorage.getItem(this.LS_SPACES)) || []; }
    catch { return []; }
  },
  saveSpaces(spaces) {
  ...
  /* ---- 邀请管理 ---- */
  getInvitations() {
    try { return JSON.parse(localStorage.getItem(this.LS_INVITATIONS)) || []; }
    catch { return []; }
  },
  ...
  /* ---- 记录 CRUD ---- */
  getRecords() {
    try { return JSON.parse(localStorage.getItem(this.LS_RECORDS)) || []; }
    catch { return []; }
  },
  getRecord(id) {
  ...
  saveBlob(id, blob, type = 'image') {
    return new Promise((resolve, reject) => {
      ...
      tx.objectStore('blobs').put({ id, blob, type, createdAt: new Date().toISOString() });
  ...
  async getStorageUsage() {
    ...
    await new Promise(resolve => {
      req.onsuccess = (e) => {
        ...
        if (cursor) { idbBytes += cursor.value.blob.size; cursor.continue(); }
        else resolve();
      };
      req.onerror = () => resolve();
    });
  ...
  async clearAll() {
    ...
    localStorage.removeItem('hl_last_sync');
    const tx = this.db.transaction('blobs', 'readwrite');
